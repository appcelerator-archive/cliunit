...
function runScript (config, opts, fn, cb, results) {
	var lines = preprocessLines(fs.readFileSync(fn).toString().split(/\n/)),
		expected = [],
		args = [],
		unit = new CLIUnit(fn),
		plugins = {};
	...
	async.eachSeries(lines, function(line, next){
		if (line) {
			...
			if (tok) {
				...
				// if the plugin requires us to be running, start
				if (plugin.requiresRunning) {
					// console.log('before',plugin.command);
					return run(function(err) {
						// console.log('after',plugin.command);
						if (err) { return next(err); }
						plugin.execute(state, token, next);
					});
				}
				else {
					// console.log('running',running,plugin.command);
					if (!running) {
						return plugin.execute(state, token, next);
					}
					else {
						// console.log('execute after running',plugin.command);
						return plugin.execute(state, token, function(err) {
							// console.log('after plugin execute',unit.pendingData.length);
							if (err) { return next(err); }
